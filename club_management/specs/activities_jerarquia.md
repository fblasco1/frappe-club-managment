...
```
Actividad (ej. Basquet, Zumba, Ritmos Latinos)
...
- **Grupo Actividad**: tira o división (Masculino / Azul, Primera División B). Cada uno puede tener su **Item** de arancel mensual.
...
Estructura unificada: ver `specs/basquet_estructura_unificada.md`.

## Scenario: actividad con grupos exige tira al inscribir

Given `Actividad` «Basquet» con `usa_grupos = 1`
And `Grupo Actividad` «Masculino / Azul» hijo de esa actividad
And `Equipo Actividad` «U11» hijo de «Masculino / Azul»
...
And «Masculino / Amarillo» con `item` = Item arancel B
...
Then existen 14 actividades habilitadas incluyendo **Basquet** (única actividad de básquet), **Zumba** y **Ritmos Latinos** como filas separadas.
