...
Then se obtiene `actividad`, `grupo` y `equipo` coherentes con la estructura seed de básquet
And Azul/Amarillo masculino apuntan a `Basquet Masculino` / `Tira Azul|Amarillo` / `U{n}`
And Flex masculino usa `Tira Flex` con equipo según edad (U15, U19 o Superior C)
And Femenino apunta a `Basquet Femenino` / `Femenino` / `U{n}` (o Superior Fem si MAYOR)
And Escuelita apunta a `Basquet Escuelita` / `Mixta` / `U7 / U9` o `U11 / U13`.
