"""Sidebar Espacios en bootinfo (Coordinacion / roles Spaces)."""
...
from club_management.spaces.setup.espacios_workspace_sidebar import SIDEBAR_ESPACIOS, SIDEBAR_ITEMS
from club_management.utils.desk_sidebar_boot import enrich_sidebar_link_item

_PANEL_ROLES = frozenset({ROLE_COORDINACION, "Secretaria", "Tesoreria", "System Manager"})


def _sidebar_boot_key() -> str:
	return SIDEBAR_ESPACIOS.lower()


def build_espacios_sidebar_boot_items() -> list[dict[str, Any]]:
...
def user_has_espacios_panel_role(bootinfo: dict[str, Any]) -> bool:
...
def apply_espacios_sidebar_to_boot(bootinfo: dict[str, Any]) -> None:
	if not user_has_espacios_panel_role(bootinfo):
		return
