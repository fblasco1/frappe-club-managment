...
MISSING_COST_CENTERS: tuple[tuple[str, str], ...] = (
	("Zumba", "Actividades - ICDPE"),
	("Basquet", "Deportes - ICDPE"),
)
...
def ensure_round_off_account() -> str:
	...
	existing = frappe.db.get_value(
		"Account",
		{"company": resolve_icdpe_company(), "account_number": ROUND_OFF_NUMBER},
		"name",
	)
	if existing:
		return existing
	...
	if not _account_exists(ACCOUNTS["otros_ingresos_parent"]):
		frappe.throw(_("Falta la cuenta padre «Otros ingresos» en {0}.").format(resolve_icdpe_company()))
...
def complete_company_defaults() -> dict[str, str | None]:
	...
	company_name = resolve_icdpe_company()
	...
	for field, value in updates.items():
		if value and frappe.db.get_value("Company", company_name, field) != value:
			frappe.db.set_value("Company", company_name, field, value, update_modified=False)
	return {k: frappe.db.get_value("Company", company_name, k) for k in updates}
...
def sync_club_settings_item_cuota() -> None:
	...
	settings.company = resolve_icdpe_company()
